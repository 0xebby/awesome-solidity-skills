...
2. **Code reuse.** Don't reinvent the wheel — reuse battle-tested libraries (OpenZeppelin, Solady).
   Reused code beats freshly written code "no matter how confident you feel." Beware
   not-invented-here.
3. **Code quality.** Treat it like aerospace, not a web widget. Rigorous methodology; assume no
   second chance to fix.
4. **Readability / auditability.** Public bytecode; write for the auditor. Clear names, documented
   invariants.
...
## Input validation — the everyday discipline
